...
class ControllerBlogArticle extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_blog_article->addArticle($this->request->post, $this->session->data['user_id']);
	...
	protected function getList() {
        ...
        if($this->session->data['user_id'] != 1){
            $results = $this->model_blog_article->getSingleArticle($filter_data, $this->session->data['user_id']);
        }else{
